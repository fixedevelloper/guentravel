...
import {
    ...
    CheckCircle2, ArrowRight, Clock, Phone, Lock
} from "lucide-react";
...
export default function FlightCheckoutPage() {
    ...
    const [momoOperator, setMomoOperator] = React.useState<string>("momo"); // 'momo' pour MTN, 'om' pour Orange Money
...
// Récupération de la mutation et de son état de chargement global
    const { mutate: checkout, isPending } = useFlightCheckout();
    // Éviter les erreurs de désynchronisation d'hydratation SSR/CSR de Zustand persisted
    React.useEffect(() => {
        setIsHydrated(true);
    }, []);
    ...
    // Le GDS Travelport+ se base sur ce montant final calculé ou transmis
    const finalTotalPrice = selectedFlight.price_details.final_price_to_pay + insurancePrice + baggagePrice;
    ...
    const handleFinalCheckout = async () => {
        ...
        const fullPhoneNumber = ["momo", "om"].includes(exactPaymentMethod)
            ? `+237${cleanedPhone}`
            : undefined;

        try {
            setIsSubmitting(true); // Active proprement le loader

            // 2. On attend la résolution complète de la promesse (réponse de Laravel)
            await checkout({
                selected_flight: {
                    ...selectedFlight,
                    ...
                    price_details: {
                        ...
                        final_price_to_pay: finalTotalPrice
                    }
                },
            ...
            console.error("Erreur durant le checkout:", error);
        } finally {
                ...
                <AnimatePresence mode="wait">
                    ...
                    {step === 1 && (
                        <motion.div key="step1" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                            ...
                            <Button onClick={() => setStep(2)} className="w-full md:w-auto bg-[#1d9e4b] hover:bg-[#167f3c] text-white font-bold px-8 h-12 rounded-xl float-right">
                    ...
                    {step === 3 && (
                        <motion.div key="step3" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                            <Card className="border-zinc-200 shadow-sm">
                                <CardHeader className="border-b border-zinc-100 bg-zinc-50/50">
                                    <CardTitle className="text-sm font-bold uppercase tracking-wider text-zinc-800 flex items-center gap-2">
                                        <CreditCard className="h-4 w-4 text-[#1d9e4b]" /> Méthode de Paiement Sécurisée
                                    </CardTitle>
                                </CardHeader>
                                    ...
                                    <RadioGroup defaultValue="momo" onValueChange={(val) => setPaymentMethod(val)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <label className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === "momo" ? "border-[#1d9e4b] bg-emerald-50/10 shadow-sm" : "border-zinc-200"}`}>
                                            ...
                                            <div>
                                                ...
                                                <span className="text-xs text-zinc-400">Débit immédiat via USSD</span>
                                            </div>
                                        </label>
                                        <label className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === "card" ? "border-[#1d9e4b] bg-emerald-50/10 shadow-sm" : "border-zinc-200"}`}>
                                            ...
                                            <div>
                                                ...
                                                <span className="text-xs text-zinc-400">Traitement international 3D Secure</span>
                                            </div>
                                        </label>
                                    </RadioGroup>
                                    ...
                                    {paymentMethod === "momo" ? (
                                        <div className="p-4 bg-zinc-50 rounded-xl border space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-[11px] text-zinc-400 flex items-center gap-1 mt-1"><Lock className="h-3 w-3" /> Vous recevrez une notification push de validation de débit sur votre mobile.</p>
                                            </div>
                                        </div>
                                    ) : (
                            ...
                            <div className="flex justify-between items-center pt-4">
                                ...
                                >
                                    {isSubmitting ? "Traitement en cours..." : `Confirmer et Payer ${finalTotalPrice.toLocaleString()} XAF`}
                                </Button>
                            </div>
                        </motion.div>
                    )}
                ...
                <Card className="border-zinc-200 shadow-sm overflow-hidden bg-white">
                    <div className="bg-zinc-900 text-white p-4">
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-[#1d9e4b]">
                            ...
                            <span className="bg-white/10 px-2 py-0.5 rounded text-white font-medium">{selectedFlight.airline_code} {selectedFlight.flight_number}</span>
                        </div>
                        <div className="mt-4 flex items-center justify-between gap-2">
                            <div>
                                <div className="text-xl font-black">{new Date(selectedFlight.departure_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                                <div className="text-xs font-bold text-zinc-400 mt-0.5">{selectedFlight.origin}</div>
                            </div>
                            <div className="flex-1 text-center px-2">
                                <span className="text-[10px] text-zinc-400 flex items-center justify-center gap-1"><Clock className="h-3 w-3" /> {formatDuration(selectedFlight.duration)}</span>
                            ...
                            <div>
                                <div className="text-xl font-black">{new Date(selectedFlight.arrival_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                    ...
                    <CardContent className="p-4 space-y-3 text-sm">
                        <div className="flex justify-between text-zinc-600"><span>Tarif de base</span><span className="font-medium">{selectedFlight.price_details.base_fare.toLocaleString()} F</span></div>
                        <div className="flex justify-between text-zinc-600"><span>Taxes aéroportuaires</span><span className="font-medium">{selectedFlight.price_details.taxes.toLocaleString()} F</span></div>
                        <div className="flex justify-between text-zinc-600"><span>Frais de service agence</span><span className="font-medium">{selectedFlight.price_details.agency_fees.toLocaleString()} F</span></div>

                        {extraBaggage > 0 && (
                            <div className="flex justify-between text-emerald-600 font-medium bg-emerald-50/40 p-1.5 rounded text-xs">
                                <span>Bagage sup. (x{extraBaggage})</span><span>+{baggagePrice.toLocaleString()} F</span>
                            </div>
                        )}
                        {insuranceSelected && (
                            <div className="flex justify-between text-blue-600 font-medium bg-blue-50/40 p-1.5 rounded text-xs">
                                <span>Assurance Multirisque</span><span>+{insurancePrice.toLocaleString()} F</span>
                            </div>
                        )}

                        <hr className="border-zinc-100 my-2" />
                        <div className="flex justify-between items-baseline pt-1">
                            <span className="font-bold text-zinc-900">Montant total</span>
                            <div className="text-right">
                                <span className="text-2xl font-black text-[#1d9e4b] tracking-tight">{finalTotalPrice.toLocaleString()}</span>
                                <span className="text-xs font-bold text-zinc-500 uppercase ml-1">{selectedFlight.price_details.currency}</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
